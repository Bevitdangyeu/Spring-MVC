...
<body>
	<div>
		<div style="margin-top:80px;display:flex;width:100%;height:80%">
			...
			<div
				style="overflow-y:scroll;font-family: 'Roboto', sans-serif;width:15%;float: left;margin-top: 20px;height:80%">
				<div class="p-1">
						...
						<ul class="list-unstyled mb-0">
							<c:forEach var="item" items="${customer }">
								<li class="p-2 border-bottom">
									<a href="#!" id="userActive${item.customerId }"
										class="d-flex justify-content-between">
										<div class="d-flex flex-row">
											<div>
												<img src="${item.img }" id="imgUserActuve${item.customerId }"
													alt="avatar" class="d-flex align-self-center me-3"
													style="width:50px;height:50px;border-radius:50px; margin-right:20px">
												<span class="badge bg-success badge-dot"></span>
											</div>
											<div class="pt-1">
												<p class="fw-bold mb-0" id="nameUserActive${item.customerId }">
													${item.customerName }</p>
											</div>
										</div>
										<div class="pt-1">
						...
						<div class="col-md-6 col-lg-5 col-xl-4 mb-4 mb-md-0">
							...
							<div>
								<div class="p-3" style="overflow-y:scroll;font-family: 'Roboto', sans-serif;height:70%">
									...
									<div id="divLastMessage" data-mdb-perfect-scrollbar-init
										style="position: relative; height: 400px"
										style="overflow-y:scroll;font-family: 'Roboto', sans-serif;height:70%">
										<!-- <ul class="list-unstyled mb-0"> -->
											...
											<c:forEach var="userChat" items="${item.user }">
												<c:if test="${userChat.customerId != user.customerId}">
													<a href="#!" class="d-flex justify-content-between"
														...
														<div class="d-flex flex-row">
															<div>
																<input type="hidden" id="img${item.conversationId }"
																	value="${userChat.img }">
																<img src="${userChat.img }" alt="avatar"
																	class="d-flex align-self-center me-3"
																	style="width:50px;height:50px;border-radius:50px; margin-right:20px">
																<span class="badge bg-success badge-dot"></span>
															</div>
															<div class="pt-1" id="divContentLastMessage">
																<p id="name${item.conversationId }"
																	class="fw-bold mb-0">${userChat.customerName }</p>
																<p class="small text-muted lastMessage"
																	id="lastMessage${item.conversationId }">
																	${item.message.content }</p>
															</div>
														</div>
														<div class="pt-1" id="divTimeLastMessage">
															<p class="small text-muted mb-1"
																id="time${item.conversationId}">${item.message.time }
															</p>
															<span
																class="badge bg-danger rounded-pill float-end">3</span>
														</div>
													</a>
													<input type="hidden" id="Participantid${item.conversationId }"
														value="${userChat.customerId }"> <!-- id người nhận -->
												</c:if>

											</c:forEach>
											<input type="hidden" id="conversationid${item.conversationId }"
								...
								<div style="width:90%;position: absolute; bottom: 0;">
									<div
										...
										<input id="messageInput" type="text" placeholder="Type message"
											style="border-radius:10px;border:1px dashed #DDDDDD;width:90%;outline:none;padding-left:15px">
										<a class="ms-1 text-muted" href="#!"><i class="fas fa-paperclip"
												style="margin-left:5px"></i></a>
										<a class="ms-3 text-muted" href="#!"><i class="fas fa-smile"
												style="margin-left:5px"></i></a>
										<a id="sendButton" class="ms-3" href="#!"><i class="fas fa-paper-plane"
												style="margin-left:5px"></i></a>
									</div>
								</div>
							</div>
	...
	<input type="hidden" id="receiverId" value=""> <!-- id người nhận sẽ được đẩy lên khi ấn vào phần tất cả user-->
	...
	<script>
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (Array.isArray(response) && response.length >
						0) { // => đã tạo cuộc trò chuyện trước đó=> có tin nhắn
						var conversation = response[0]
							.conversationId // => ghi giá trị của mã đoạn chat lên để đánh dấu đang ở đoạn chat nào
						console.log(' conversationId ' + conversation);
						$('#conversationID').val(conversation);
						$('#receiverId').val(userActiveId);
						displayMessages(response);
					} else { // chưa từng trò chuyện => không có tin nhắn
		...
		// hàm thao tác xóa tin nhắn => vì đây là các phần tử được thêm động nên không thể gọi trực tiếp button[id^="delete"] mà phải thông qua phần tử cha
		...
		// hàm kích hoạt sửa tin nhắn
		$('#messages').on('click', 'button[id^="edit"]', function () {
			// lấy giá trị id của tin nhắn đó
			var messageId=$(this).attr('id').replace('edit','')
			var edit = document.getElementById("edit"+ messageId);
			edit.style.display="none";
			// lấy ra div chứa tin nhắn
			var divMess = document.getElementById("time"+ messageId);
				var div =document.createElement('div');
				div.style.display="flex";
				var yes = document.createElement('a');
				yes.innerHTML ='<i class="fa fa-check" aria-hidden="true"></i>';
				yes.href="#";
				yes.id="ChooseEdit"+messageId;
				yes.style.marginRight="10px";
				yes.style.marginLeft="10px";
				var no = document.createElement('a');
				no.innerHTML ='<i class="fa fa-times" aria-hidden="true"></i>';
				no.id="ChonHuy"+messageId;
				no.href="#";
				
				// thêm 2 phần tử vào thẻ div
				div.appendChild(yes);
				div.appendChild(no);
			divMess.appendChild(div);
			//loại bỏ readonly của tin nhắn và thêm dấu nháy chuột
			//lấy phần tử đó ra
			var inputField = document.getElementById("messageContent" + messageId);
			inputField.removeAttribute("readonly"); // xóa readonly
			// đặt dấu nháy chuột
			inputField.focus();
		
		});
		// hàm sửa tin nhắn
		$('#messages').on('click', 'a[id^="ChooseEdit"]', function () {
			var messageId=$(this).attr('id').replace('ChooseEdit','');
			var content =$('#messageContent'+messageId).val();
			var conversationId=$('#conversationID').val();
			console.log(content);  
			var edit = document.getElementById("edit"+ messageId);
			...
			var yes = document.getElementById("ChooseEdit" + messageId);
			yes.style.display="none";
			var no=document.getElementById("ChonHuy" + messageId);
			no.style.display="none";
			$.ajax({
				url: '/SpringMVC2/user/edit',
				type: 'post',
				data: JSON.stringify({idMessenge:messageId,content:content,conversationId:conversationId}),
				contentType: 'application/json',
				dataType: 'json',
				success: function (response) {
					console.log(response);
					var inputField = document.getElementById("messageContent" + messageId);
					inputField.value=response.content;
				},
				error: function (response) {
					console.error(response);
				}
		...
		function showMessage(message) {
			...
			var sendName = message.sender.customerName; // Tên người gửi
			var senderID = message.sender.customerId;
			var content = message.content; // Nội dung tin nhắn
			...
			if (ContentlastMessage) {
				...
				a.href = "#";
				// tạo một thẻ input để ghi id của người nhận lên  idReceiver
				const inputIdReceiver = document.createElement('input');
				inputIdReceiver.type="hidden";
				inputIdReceiver.value=idReceiver;
				inputIdReceiver.id="Participantid"+conversationId;
			...
			edit.id = 'edit' + idMess;
			// tạo id cho new Message
			newMessage.id='messageContent'+idMess;
			...
			if (senderID == currentUser) {
				...
				Elementtime.style.display = 'flex';
